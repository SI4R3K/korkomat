...
class UserExceptionHandler {
    ...
    fun handleUserNotFoundException(ex: Exception): ResponseEntity<Api<Any>> {
        val errorResponse = Api.error<Any>(
            message = ex.message ?: Constant.USER_NOT_FOUND,
    ...
    fun handleUserProfileAlreadyExistsException(ex: Exception): ResponseEntity<Api<Any>> {
        val errorResponse = Api.error<Any>(
            message = ex.message ?: Constant.PROFILE_ALREADY_EXISTS,
            errorStatus = ErrorStatus.NOT_FOUND
        )
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(errorResponse)
    }
